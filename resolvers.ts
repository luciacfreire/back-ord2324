...
export const resolvers = {
    ...
    Mutation: {
        deleteContact: async (
            _: unknown,
            args: DeleteContactMutationArgs,
            ctx: Context
        ) : Promise<boolean> => {
        ...
        updateContact: async(
            ...
            //Verificar duplicados si el teléfono cambió 
            if(phone && phone !== contact.phone){
                const phoneExists = await ctx.ContactsCollection.findOne({phone});
                if(phoneExists) throw new GraphQLError("Phone is already taken");

                //Validar el nuevo numero
                const url = `https://api.api-ninjas.com/v1/validatephone?number=${phone}`;
                const data = await fetch(url,
                    {
                        headers: {
                ...
                //Actualizar los campos relacionados con el telefono
                contact.phone = phone;
                contact.country = response.country;
                contact.timezone = response.timezones[0];
            }
            ...
            const updateContact = await ctx.ContactsCollection.findOneAndUpdate(
                {_id: new ObjectId(id)},
                {
                 $set:{
                    name: contact.name,
            ...
            if(!updateContact) throw new GraphQLError("Error updating contact");
